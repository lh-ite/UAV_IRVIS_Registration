function [matchedPoints1, matchedPoints2]=fineRegistrationByMcog(im1_gray, im2_gray, batch_size, point_refine)
% size(im1_gray);
% size(im2_gray)
% feature_Ref = CFOG(single(im1_gray));
% feature_Sen = CFOG(single(im2_gray));
tic
feature_Ref = mcogFeature(single(im1_gray));

feature_Sen = mcogFeature(single(im2_gray));
toc
