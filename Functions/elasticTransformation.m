function [mosaic_img, reg_img, u_, v_] = elasticTransformation(im1, im2, X1, X2)
im_ch = size(im1,3);
imsize1 = size(im1);
imsize2 = size(im2);
[ ~, ok, ~ ] = homographyRansac(X1, X2, 500, 0.1);
...
[paras_init,~,~,~,~] = lsqnonlin(...
    @(p)robustKernelResidual(double(X1_ok), double(X2_ok), imsize1, imsize2, p, 1000), [1000 1000 0 0 0],...
    [],[],options);
[paras,~,~,~,~] = lsqnonlin(...
    @(p)robustKernelResidual(double(X1_ok), double(X2_ok), imsize1, imsize2, p, 10), paras_init,...
    [],[],options);
k1 = paras(1);
k2 = paras(2);
theta = paras(3:5);
...
if size(im2_p,1)~=size(im1_p,1)
    im2_p = imresize(im2_p, size(im1_p, 1:2), 'Method', 'bilinear');
end
mask1 = ~isnan(im1_p);
mask2 = ~isnan(im2_p);
mass = mask1 + mask2 ;
